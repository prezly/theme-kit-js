...
const subPackages = rootPackage.workspaces.map((packagePath) => {
...
const updatedSubPackages = subPackages
    .map((subPackage) => {
        let contents = subPackage.contents;

        if (workspace) {
            contents = subPackages.reduce((contents, _sub) => {
                return contents.replace(
                    /"version":\s+"([^"]+)"/g,
                    (matchedString, matchedVersion) =>
                        matchedString.replace(matchedVersion, monorepoVersion),
                );
            }, contents);
        ...
        contents = packages.reduce((contents, packageName) => {
            ...
            return contents.replace(
                ...
                (matchedString, matchedVersion) =>
                    matchedString.replace(matchedVersion, packageVersion),
            );
        }, contents);
